--@module "plenary.busted"

-- luacheck: globals describe it assert
local _99 = require("99")
local test_utils = require("99.test.test_utils")
local Levels = require("99.logger.level")
local eq = assert.are.same

--- @param content string[]
--- @param row number
--- @param col number
--- @param lang string?
--- @return _99.test.Provider, number
local function setup(content, row, col, lang)
  assert(lang, "lang must be provided")
  local provider = test_utils.TestProvider.new()
  _99.setup({
    provider = provider,
    logger = {
      error_cache_level = Levels.ERROR,
      type = "print",
    },
  })

  local buffer = test_utils.create_file(content, lang, row, col)
  return provider, buffer
end

--- @param buffer number
--- @return string[]
local function read(buffer)
    return vim.api.nvim_buf_get_lines(buffer, 0, -1, false)
end

describe("fill_in_function", function()
    it("fill in csharp method", function()
        local csharp_content = {
            "",
            "public int Test() { }",
        }
        local provider, buffer = setup(csharp_content, 2, 15, "csharp")
        local state = _99.__get_state()
        _99.fill_in_function()
        eq(1, state:active_request_count())
        eq(csharp_content, read(buffer))
        provider:resolve("success", "public int Test()\n{\n    return 42;\n}")
        test_utils.next_frame()
        local expected_state = {
            "",
            "public int Test()",
            "{",
            "    return 42;",
            "}",
        }
        eq(expected_state, read(buffer))
        eq(0, state:active_request_count())
    end)

    it("fill in csharp method with comment", function()
        local csharp_content = {
            "",
            "public int Calculate(int x, int y)",
            "{",
            "    // TODO: return the sum",
            "}",
        }
        local provider, buffer = setup(csharp_content, 2, 15, "csharp")
        local state = _99.__get_state()
        _99.fill_in_function()
        eq(1, state:active_request_count())
        eq(csharp_content, read(buffer))
        provider:resolve("success", "public int Calculate(int x, int y)\n{\n    return x + y;\n}")
        test_utils.next_frame()
        local expected_state = {
            "",
            "public int Calculate(int x, int y)",
            "{",
            "    return x + y;",
            "}",
        }
        eq(expected_state, read(buffer))
        eq(0, state:active_request_count())
    end)

    it("fill in async csharp method", function()
        local csharp_content = {
            "",
            "public async Task<string> FetchDataAsync() { }",
        }
        local provider, buffer = setup(csharp_content, 2, 25, "csharp")
        local state = _99.__get_state()
        _99.fill_in_function()
        eq(1, state:active_request_count())
        eq(csharp_content, read(buffer))
        provider:resolve("success",
            "public async Task<string> FetchDataAsync()\n{\n    return await Task.FromResult(\"data\");\n}")
        test_utils.next_frame()
        local expected_state = {
            "",
            "public async Task<string> FetchDataAsync()",
            "{",
            "    return await Task.FromResult(\"data\");",
            "}",
        }
        eq(expected_state, read(buffer))
        eq(0, state:active_request_count())
    end)
end)
