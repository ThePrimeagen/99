...
function M.get_preceding_comments(context, func_range)
    ...
    for id, node, _ in query:iter_captures(root, buffer, 0, -1, { all = true }) do
        ...
        then
            ...
            -- Note: tree-sitter end positions are exclusive, so a comment on line N
            -- may have end_.row = N+1. We use <= to handle the case where the
            -- comment ends exactly at the function start row.
            if
...
function M.get_enclosing_context(context, cursor)
    ...
    for id, node, _ in query:iter_captures(root, buffer, 0, -1, { all = true }) do
        ...
        if range:contains(cursor) then
            -- Only match top-level context captures like "context.impl", not "context.impl.body"
            -- Pattern matches "context.word" but not "context.word.subword"
            ...
            elseif kind and enclosing[kind] then
                ...
                if range:area() < enclosing[kind].range:area() then
                    enclosing[kind] =
                        { node = node, range = range, capture = name }
                end
            end
        end
    end
