...
-- Functions for gathering LSP context to improve LLM decision-making
--------------------------------------------------------------------------------
...
function M.get_type_info(buffer, identifiers, cb)
    ...
    for _, ident in ipairs(identifiers) do
        ...
        get_lsp_hover(buffer, position, function(result, _)
            if result and result.contents then
                local content = result.contents
                local hover_text
                ...
                if hover_text then
                    hover_text =
                        hover_text:gsub("```%w*\n?", ""):gsub("```", "")
                    hover_text = hover_text:gsub("^%s+", ""):gsub("%s+$", "")
                    if hover_text ~= "" then
...
-- Export both the original Lsp class and the new context gathering functions
return {
    Lsp = Lsp,
    has_lsp = M.has_lsp,
    extract_identifiers = M.extract_identifiers,
    get_type_info = M.get_type_info,
    format_type_context = M.format_type_context,
    gather_context = M.gather_context,
    gather_context_sync = M.gather_context_sync,
}
